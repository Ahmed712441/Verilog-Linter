...
class CaseNode:
    ...
    def __init__(self,line,size,name,children):
        self.line = line
        self.size = size
        self.name = name
    ...
    def __repr__(self) -> str:
        return self.children.__repr__() + ' ' +self.name + '_' + str(self.line) + '_' + str(self.size)
...
def getChildren(tokens:list,i:int):
    
    children = []
    while i < len(tokens):
        token = tokens[i]
        if token == ':' and tokens[i+1] == 'begin':
            children.append(tokens[i-1])
            i+=1
        elif token == 'endcase':
            break
        i+=1
...
def getCases(tokens:list,variables:dict)->list:
    ...
    while i < len(tokens):
        ...
        if token == '\n':
            ...
            caseName = token
            line = current_line
            children , i = getChildren(tokens,i)
            caseNode = CaseNode(line,caseSize,caseName,children)
